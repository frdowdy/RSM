...
cycle_5_significant <- cycle_5_data[,"43003.56"] 
summary(rsm5)
persp(rsm5, ~ temp + acetate, col = rainbow(50), contours = "colors")
contour(rsm5, ~ temp + acetate, col = rainbow(50), contours = "colors")
...
rsm_cols$type <- gsub("valdiation","validation",rsm_cols$type)
...
```
Let's try analyzing charge_C as well. Doesn't fit well to our response surface.
```{r}
current_output %>% filter(cycle==5) %>% select(mfc,charge_C) -> data 

trans = preProcess(data.frame(data %>% select(-1)), c("BoxCox", "center", "scale"))
data_trans = data.frame(trans = predict(trans, data.frame(data %>% select(-1))))
colnames(data_trans) <- paste("trans_",colnames(data_trans),sep = "")
# # Visualize again
# data_trans %>% apply(.,2,hist) 
# data_trans %>% apply(.,2,ggqqplot)
# data_trans %>% apply(.,2,shapiro.test)



# Attach transformed variables to data set
rsm_hist <- rsm_cols
rsm_hist <- cbind(rsm_hist,data_trans)


#fit data to response surface

rsm <- rsm(rsm_hist$trans_charge_C ~ SO(temp, acetate), data = rsm_hist)
summary(rsm)
persp(rsm, ~ temp + acetate, col = rainbow(50), contours = "colors")
```
...
IVIS Image data
```{r get IVIS data}
read_tsv('./IVIS/ivis_measurements.txt') -> ivis
ivis[ivis==""] <- NA #Convert blank values to NA
ivis[ivis=="N/A"] <- NA
ivis %>% select(which(colMeans(is.na(.)) < 0.5)) -> ivis #select columns with less than 50% NA
colnames(ivis) %>% str_replace_all(" ","_") -> colnames(ivis)
```

```{r put in channels for easy spreading}
# From JaeB Kim: In terms of the fluorescence unit, we recommend “Total Radiant Efficiency” than Avg Radiant Efficiency. Avg radiant efficiency considers pixel number in the calculation. Sometimes depending on how you draw ROI’s, the measurements may get affected. Total radiant efficiency is simply the sum of the signals in the ROI. That usually works in most cases including yours.

ivis %>% mutate(Channel = ifelse(EM_Filter==540,"SG","PI")) %>%  mutate(Side=unlist(str_extract_all(Comment1,"[a-z]"))) %>% 
  mutate(mfc=unlist(str_extract_all(Comment1,"[0-9]{1,2}"))) %>% 
  select(mfc,Side,Channel,`Total_Radiant_Efficiency_[p/s]_/_[uW/cm2]`) %>% spread(Channel,`Total_Radiant_Efficiency_[p/s]_/_[uW/cm2]`) -> data
#How much do front and back readings differ for the same MFC? When blocking by MFC, back fluorescence always higher!
anova(lm(PI~Side+mfc,data))
anova(lm(SG~Side+mfc,data)) 
  
  # summarize by finding mean of front and back
  ivis %>% mutate(Channel = ifelse(EM_Filter==540,"SG","PI")) %>%  mutate(Side=unlist(str_extract_all(Comment1,"[a-z]"))) %>% 
  mutate(mfc=unlist(str_extract_all(Comment1,"[0-9]{1,2}"))) %>% 
  select(mfc,Side,Channel,`Total_Radiant_Efficiency_[p/s]_/_[uW/cm2]`) %>% spread(Channel,`Total_Radiant_Efficiency_[p/s]_/_[uW/cm2]`) %>%         group_by(mfc) %>% summarise_each(funs(mean)) %>% select(-Side) %>% mutate("live/dead"=(SG-PI)/PI) %>% mutate_each(funs(as.numeric(.))) %>% arrange(mfc) -> ivis_data
```
Now let's see if we can fit our fluorescence data to our response surface. Note that increasing temperatures decrease SG, PI, and the live/dead ratio. Additionally acetate^2 has a significant effect on SG and PI signals across the mfcs.
```{r fluorescent data analysis}
data <- ivis_data


# #visualize
# data %>% select(-1) %>% apply(.,2,hist) 
# data %>% select(-1) %>% apply(.,2,ggqqplot)
# data %>% select(-1) %>% apply(.,2,shapiro.test)
# shapiro.test(current_output$ce) # if p<.05, reject null hypothesis that data is normal, there is evidence data not normal
#preprocess
trans = preProcess(data.frame(data %>% select(-1)), c("BoxCox", "center", "scale"))
data_trans = data.frame(trans = predict(trans, data.frame(data %>% select(-1))))
...
rsm <- rsm(rsm_hist$trans.PI ~ SO(temp, acetate), data = rsm_hist)
summary(rsm)
persp(rsm, ~ temp + acetate, col = rainbow(50), contours = "colors")

rsm <- rsm(rsm_hist$trans.SG ~ SO(temp, acetate), data = rsm_hist)
summary(rsm)
persp(rsm, ~ temp + acetate, col = rainbow(50), contours = "colors")

rsm <- rsm(rsm_hist$trans.live.dead ~ SO(temp, acetate), data = rsm_hist)
summary(rsm)
persp(rsm, ~ temp + acetate, col = rainbow(50), contours = "colors")
```
Next, let's do multivariate analysis on all the dependent variables in rsm_hist and see if any are significantly correlated.
```{r}
rsm_hist
names(rsm_hist)[names(rsm_hist) == 'run'] <- 'mfc'
#Get delta COD
cod$mfc <- as.numeric(cod$mfc)
cod %>% filter(cycle==5) %>% select(mfc,delta_cod_mg_L,delta_cod_mg_L_day) %>% full_join(rsm_hist,.) -> rsm_hist
#Voltage
cycle_5_data[,"43003.56"] %>% cbind(gsub("X","",rownames(cycle_5_data,.))) %>% as_tibble() -> voltage
colnames(voltage) <- c("voltage","mfc")
voltage %>% mutate_all(as.numeric) %>% full_join(rsm_hist,.) -> rsm_hist
#Energy production
names(power_output_rsm)[names(power_output_rsm) == 'run'] <- 'mfc'
names(power_output_rsm)[names(power_output_rsm) == '5'] <- 'cycle_5_energy_J'
names(power_output_rsm)[names(power_output_rsm) == 'cumulative'] <- 'cumulative_energy_J'
power_output_rsm %>% select(mfc,cycle_5_energy_J,cumulative_energy_J) %>% full_join(rsm_hist,.) -> rsm_hist
#CE
current_output %>% filter(cycle==5) %>% arrange(mfc) %>% select(mfc,ce_trans) %>% full_join(rsm_hist,.) -> rsm_hist
names(rsm_hist)[names(rsm_hist) == 'ce_trans'] <- 'trans_ce'
#transform any untransformed
colnames(rsm_hist) <- str_replace_all(colnames(rsm_hist),"[.]","_")
rsm_hist %>% as_tibble() %>% select(-c(type,temp,acetate)) %>% select(-starts_with("trans")) -> data
...
rsm_hist <- cbind(rsm_hist,data_trans)
colnames(rsm_hist) <- str_replace_all(colnames(rsm_hist),"[.]","_")
rsm_hist %>% as_tibble() %>% select(-c(mfc,type,temp,acetate)) %>% select(starts_with("trans")) -> rsm_hist_trans
```
Now that we've got the data let's do multivariate analysis.
Note the p matrix isn't symmetric for corr.test(...,adjust="BH")
```{r, fig.height=7, fig.width=7}
# https://cran.r-project.org/web/packages/corrplot/vignettes/corrplot-intro.html
packages(psych)
corr.test(rsm_hist_trans,adjust = "none") -> trans_corr
...
corrplot(trans_corr$r, type = "lower", order = "hclust", p.mat = trans_corr$p, sig.level = 0.05, insig = "blank")

#make diagonal excluded
diag(trans_corr$p) <- 1
corrplot(trans_corr$r, type = "full", method = "ellipse",order = "hclust", p.mat = trans_corr$p, sig.level = 0.05, insig = "blank",tl.col = "black", tl.srt = 45)
```
Let's check and make sure we get the same BH values in JMP. Slightly different, but the same number of significant values.
```{r}
# lower.tri(trans_corr$p)*trans_corr$p -> data
# as.vector(data) -> data
# data <- data[data != 0]
...
What happens if we only select the center point replicates? Do we get better correlation? No.
```{r}
# rsm_hist %>% filter(type=="center") %>% select(starts_with("trans")) ->rsm_center_hist_trans
# 
# 
# corr.test(rsm_center_hist_trans,adjust = "none") -> trans_corr
# #non-adjusted p value matrix is symmetric, BH is not
# p.adjust(trans_corr$p,method = "BH") %>% matrix(.,nrow = nrow(trans_corr$r),ncol = ncol(trans_corr$r)) -> trans_corr$p
# packages(corrplot)
# packages(RColorBrewer)
...
# #make diagonal excluded
# diag(trans_corr$p) <- 1
# corrplot(trans_corr$r, type = "full", method = "ellipse",order = "hclust", p.mat = trans_corr$p, sig.level = 0.05, insig = "blank",tl.col = "black")
```

What if we do PCA/LDA? Not getting significant Wilks Lambda, so the results aren't clustering too well based on type, temp, or acetate.
```{r}
# rsm_hist %>% select(mfc,type,temp,acetate,starts_with("trans")) -> data # doesn't look too promising in JMP
# 
# # Wilks Lambda
# dep.vars <- cbind(data %>% select(-mfc,-type,-temp,-acetate))
# manova_fit <- manova(as.matrix(dep.vars) ~ data$type)
# summary(manova_fit, test="Wilks",tol = 0)
# manova_fit <- manova(as.matrix(dep.vars) ~ as.factor(data$temp))
# summary(manova_fit, test="Wilks",tol = 0)
# manova_fit <- manova(as.matrix(dep.vars) ~ as.factor(data$acetate))
# summary(manova_fit, test="Wilks",tol = 0)
```
Now let's go back and see if we missed anything in our response surfaces. Let's fit all the transformed variables to temp and acetate, with and without the validation points.
```{r}
# with validation points
# https://stackoverflow.com/questions/22713325/fitting-several-regression-models-with-dplyr
rsm_hist %>% select(mfc,type,temp,acetate,starts_with("trans"))  %>% gather("variable","value",c(5:ncol(.))) %>% group_by(variable) %>% do(fitVariable = rsm(value ~ SO(temp, acetate), data = .)) -> dfVariable

# get the coefficients by group in a tidy data_frame
dfVariableCoef = tidy(dfVariable, fitVariable)
dfVariableCoef
dfVariableCoef %>% filter(p.value<=.05,term != "(Intercept)")

# get the predictions by group in a tidy data_frame - doesn't work
...
dfVariableSumm
```
I'm getting the same significant RSM terms in JMP, which is good.
Code runs well so far.

```{r all rsm without validation points}
# without validation points
# https://stackoverflow.com/questions/22713325/fitting-several-regression-models-with-dplyr
rsm_hist %>% select(mfc,type,temp,acetate,starts_with("trans"))  %>% filter(type!="validation") %>% gather("variable","value",c(5:ncol(.))) %>% group_by(variable) %>% do(fitVariable = rsm(value ~ SO(temp, acetate), data = .)) -> dfVariable
...
dfVariableCoef %>% filter(p.value<=.05,term != "(Intercept)") %>% select(variable) %>% unique() %>% as.matrix() %>% match(.,dfVariable$variable) -> sig_models
dfVariable$variable[sig_models]

# loop it and provide titles
for(i in sig_models){
 dfVariable$fitVariable[i] %>% unlist(recursive = FALSE) %>% structure(class = "lm") %>% 
  persp(., ~ temp + acetate, col = rainbow(50), contours = "colors")
  title(main = paste(dfVariable$variable[i],"excluding validation points"))
}
```
Very nice!
It may be worth coming back to this type of analysis (RSM and multivariate) after getting the sequencing data in - like Shannon's index of bristles, RA of different otu's, etc.

Take out non-significant RSM terms in models, re-visualize (do by turning coefficient estimate into zero? AIC selection?)
```{r find best models with step()}
# without validation points, then stepped with AIC
rsm_hist %>% select(mfc,type,temp,acetate,starts_with("trans")) %>% filter(type!="validation") %>% gather("variable","value",c(5:ncol(.))) %>% group_by(variable) %>% do(fitVariable = step(trace=0,lm(value ~ temp+acetate+I(temp^2)+I(acetate^2)+temp:acetate, data = .))) -> dfVariable
...
How well do each of these models predict the validation points?
```{r model validation}
rsm_hist %>% select(mfc,type,temp,acetate,dfVariable$variable[sig_models])  %>% filter(type=="validation") -> new.data

#Try with one model
# predict(dfVariable$fitVariable[3],newdata = new.data, interval = "predict") 

#Try with all models
rsm_hist %>% select(mfc,type,temp,acetate,dfVariable$variable[sig_models])  %>% filter(type=="validation") %>% gather("variable","value",c(5:ncol(.))) %>% group_by(variable) %>% 
  bind_cols(.,data.frame(do.call(rbind, predict(dfVariable$fitVariable[sig_models],newdata = new.data, interval = "predict")))) %>%    mutate(good_prediction = value>lwr & value<upr) %>% write_csv("validation.csv")
```
What are the optimal conditions for each response surface? Local vs. global? Inside or outside design space?
```{r}
df <- as_tibble(list(model=NA,temp=NA,acetate=NA,value=NA,maxmin=NA,locglo=NA))

newdat = data.frame(temp = rep(seq(min(rsm_hist$temp),max(rsm_hist$temp),length.out = 1000),each=1000))
newdat$acetate = rep(seq(min(rsm_hist$acetate),max(rsm_hist$acetate),length.out = 1000),times = 1000)
...
for(i in sig_models){
  ...
  as_tibble(list(model=dfVariable$variable[i],temp=newdat$temp[which.min(newdat$pred)],acetate=newdat$acetate[which.min(newdat$pred)],value=min(newdat$pred),maxmin="min",locglo="loc")) %>% bind_rows(df,.) -> df 
  
 bind_rows(df,as_tibble(list(model=dfVariable$variable[i],temp=newdat$temp[which.max(newdat$pred)],acetate=newdat$acetate[which.max(newdat$pred)],value=max(newdat$pred),maxmin="max",locglo="loc"))            ) ->df
...
df
```
What about the global max mins? This has to be done manually because of the difference in models. Note: this method doesn't work for interaction terms.
```{r}
# http://r.789695.n4.nabble.com/how-to-compute-maximum-of-fitted-polynomial-td4668659.html
packages(polynom)

# loop and output in table
glodf <- as_tibble(list(model=NA,
  indepvar=NA,
  indepvalue=NA,
  depvalue=NA,
  maxmin=NA,
...
dfVariable$fitVariable[i] %>% unlist(recursive = FALSE) %>% structure(class = "lm") -> model
model %>% tidy %>% select(term,estimate) %>% filter(str_detect(term,'Intercept|temp')) %>% .$term == "temp" -> test

model %>% tidy %>% select(term,estimate) %>% filter(str_detect(term,'Intercept|temp')) -> dat
ifelse(any(test) == FALSE, 
      
      rbind(dat[1,],c("temp",0),dat[2,]) %>% select(estimate) %>% unlist %>% na.omit %>% polynomial->p0
      ,
      model %>% tidy %>% select(term,estimate) %>% filter(str_detect(term,'Intercept|temp')) %>% select(estimate) %>% unlist %>% polynomial->p0
      )
...
as_tibble(list(
  model=dfVariable$variable[i],
  indepvar="temp",
  indepvalue=xmax,
  depvalue=predict(p0,xmax),
  maxmin="max",
  locglo="glo")) %>% rbind(glodf) -> glodf

xmin = xm[predict(p2, xm) > 0] # select the minima
as_tibble(list(
  model=dfVariable$variable[i],
  indepvar="temp",
  indepvalue=xmin,
  depvalue=predict(p0,xmin),
  maxmin="min",
  locglo="glo")) %>% rbind(glodf) -> glodf

model %>% tidy %>% select(term,estimate) %>% filter(str_detect(term,'Intercept|acetate')) %>% .$term == "acetate" -> test
model %>% tidy %>% select(term,estimate) %>% filter(str_detect(term,'Intercept|acetate')) -> dat
ifelse(any(test) == FALSE, 
      
      rbind(dat[1,],c("acetate",0),dat[2,]) %>% select(estimate) %>% unlist %>% na.omit %>% polynomial->p0
      ,
      model %>% tidy %>% select(term,estimate) %>% filter(str_detect(term,'Intercept|acetate')) %>% select(estimate) %>% unlist %>% polynomial->p0
      )
p1 <- deriv(p0); p2 <- deriv(p1) # first and second derivative
xm <- solve(p1) # maxima and minima of p0
xmax = xm[predict(p2, xm) < 0] # select the maxima
...
glodf %>% filter(model!="NA") -> glodf
glodf<-glodf[dim(glodf)[1]:1,] #reverse the order

#note - the following is a pretty ugly workaround
glodf %>% spread(indepvar,indepvalue) %>% filter(!is.na(depvalue)) %>% group_by(model,maxmin) %>% fill(temp,acetate) %>% fill(temp,acetate,.direction = "up") %>% select(model,temp,acetate,maxmin,locglo) %>% distinct(model,temp,acetate,maxmin,locglo) %>% group_by(model) %>% fill(temp,acetate) %>% fill(temp,acetate,.direction = "up") %>% distinct(model,.keep_all = TRUE) %>% mutate(value = diag(as.matrix(do.call(rbind,predict(dfVariable$fitVariable[match(.$model,dfVariable$variable)],newdata=.))))) %>% bind_rows(df) %>% write_csv("glolocmaxmin.csv")
```

What does the DNA analysis look like?
```{r}
