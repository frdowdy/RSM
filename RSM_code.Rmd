...
# # try using step on a single rsm model to verify method -  note "rsm" modeling method groups FO and PQ terms together, not good for aic
# rsm_hist %>% select(mfc,type,temp,acetate,trans_voltage)  %>% filter(type!="validation") %>% gather("variable","value",c(5:ncol(.))) %>% group_by(variable) %>% do(fitVariable = lm(value ~ I(temp^2)+I(acetate^2)+temp+acetate+temp*acetate, data = .)) -> dfVariable
# tidy(dfVariable, fitVariable)
# #now with step
# rsm_hist %>% select(mfc,type,temp,acetate,trans_voltage)  %>% filter(type!="validation") %>% gather("variable","value",c(5:ncol(.))) %>% group_by(variable) %>% do(fitVariable = step(trace=0,lm(value ~ I(temp^2)+I(acetate^2)+temp+acetate+temp*acetate, data = .))) -> dfVariable
# tidy(dfVariable, fitVariable)
# # seems to work well

# without validation points, then stepped with AIC
rsm_hist %>% select(mfc,type,temp,acetate,starts_with("trans")) %>% filter(type!="validation") %>% gather("variable","value",c(5:ncol(.))) %>% group_by(variable) %>% do(fitVariable = step(trace=0,lm(value ~ I(temp^2)+I(acetate^2)+temp+acetate+temp*acetate, data = .))) -> dfVariable
...
dfVariableCoef %>% filter(p.value<=.05,term != "(Intercept)") %>% select(variable) %>% unique() %>% as.matrix() %>% match(.,dfVariable$variable) -> sig_models
dfVariable$variable[sig_models]
# loop it, visualize, and provide titles
# dfVariable$fitVariable[1] %>% unlist(recursive = FALSE) %>% structure(class = "lm") -> model
#   persp(model, ~ temp + acetate, col = rainbow(50), contours = "colors")
#   title(main = paste(dfVariable$variable[i],"excluding validation points"))

# loop it and provide titles - looks like it doesn't know what to do when trans or acetate coefficients are undefined
for(i in sig_models){
 dfVariable$fitVariable[i] %>% unlist(recursive = FALSE) %>% structure(class = "lm") %>%
  persp(., ~ temp + acetate, col = rainbow(50), contours = "colors")
  title(main = paste(dfVariable$variable[i],"excluding validation points"))
...
```
What are the optimum conditions for each response surface?
```{r}
