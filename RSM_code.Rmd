...
```{r remove non-significant terms}
# without validation points, rsm package
rsm_hist %>% dplyr::select(mfc,type,temp,acetate,trans_cumulative_energy_J) %>% filter(type!="validation") -> data
rsm(trans_cumulative_energy_J ~ SO(temp, acetate), data = data) %>% step()

#regular lm fit - seems to work better with step()
rsm_hist %>% dplyr::select(mfc,type,temp,acetate,trans_cumulative_energy_J) %>% filter(type!="validation") -> data
lm(trans_cumulative_energy_J ~ I(temp*2)+I(acetate^2)+temp+acetate+temp*acetate, data = data) %>% step()

# without validation points, filtered for only the significant models, then stepped with AIC
dfVariableCoef %>% filter(p.value<=.05,term != "(Intercept)") %>% select(variable) %>% unique() %>% as.matrix() -> sig_models_names
rsm_hist %>% select(mfc,type,temp,acetate,sig_models_names)  %>% filter(type!="validation") %>% gather("variable","value",c(5:ncol(.))) %>% group_by(variable) %>% do(fitVariable = step(lm(value ~ I(temp*2)+I(acetate^2)+temp+acetate+temp*acetate, data = .))) -> dfVariable
...
# get all significant models (already selected only significant ones)
dfVariableCoef %>% select(variable) %>% unique() %>% as.matrix() %>% match(.,dfVariable$variable) -> sig_models

# loop it, visualize, and provide titles - why isn't this working?
...
```{r model validation}
rsm_hist %>% select(mfc,type,temp,acetate,starts_with("trans"))  %>% filter(type=="validation") -> new.data
predict(dfVariable$fitVariable[1],newdata = new.data, interval = "confidence") #note this predicts trans_ce using the full rsm for my two validation points
